  ...
  return (
    <div className="space-y-8 relative">
      ...
      <div className="flex justify-between items-center">
        <h1 className="text-4xl font-extrabold tracking-tight heading-gradient">Admin Dashboard</h1>
        <span className="bg-red-50 text-red-600 border border-red-100 text-xs font-bold px-4 py-1.5 rounded-full shadow-sm">ADMIN ACCESS</span>
      </div>
            ...
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-gray-400 uppercase tracking-wider text-xs">
                  ...
                  <th className="pb-4 font-semibold">Logs</th>
                        ...
                        <div className="flex items-center justify-center gap-2">
                          ...
                          {user.role !== 'ADMIN' && (
                            <form action={deleteUser.bind(null, user.id)}>
                              <button className="text-red-500 hover:text-red-700 text-xs font-medium bg-red-50 hover:bg-red-100 px-3 py-2 rounded-xl transition-colors border border-red-100">Delete</button>
                            </form>
                          )}
                        </div>
                    </td>
            ...
            <table className="min-w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100 text-gray-400 uppercase tracking-wider text-xs">
                  ...
                  <th className="pb-4 font-semibold">Usage</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {exercises.map((ex: any) => (
                  <tr key={ex.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="py-4 text-gray-600">{ex._count.logs} logs</td>
                    <td className="py-4">
                      <form action={deleteExercise.bind(null, ex.id)}>
                        <button className="text-red-500 hover:text-red-700 text-xs font-medium bg-red-50 hover:bg-red-100 px-3 py-1 rounded-md transition-colors">Delete</button>
                      </form>
                    </td>
                  </tr>
                ))}
